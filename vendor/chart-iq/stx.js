...
(function(){
	...
	function _stx_js(_exports, _stxThirdParty) {
		...
		STX.isIOS8 = navigator.userAgent.match(/iPad;.*CPU.*OS 8_\d/i);
		STX.isIOS7or8 = STX.isIOS7 | STX.isIOS8;
		...
		*/
		 ...
		 * @param  {requestCallback} params.cb          Callback function when complete
...
*/});
		 ...
		 * @param  {string}   symbol The symbol
		 * @param  {boolean}  inverse Set to true to test currency/metal pair e.g.^USDXAU
		...
		STX.LegacyMarket.isForexMetal=function(symbol,inverse){
			...
			if(",XAU,XPD,XPT,XAG,".indexOf(","+symbol.substr((inverse?4:1),3)+",")!=-1) return true;
			return false;
		};
		 ...
		 * @memberOf STX.Studies
		 */
		STX.Studies.fillArea=function(stx, points, color, opacity){
		...
		STX.Studies.initializeFN=function(stx, type, inputs, outputs, parameters, panel){
			...
			if(parameters.replaceID && (stx.panelExists(parameters.replaceID) || isOverlay)){
				if(isOverlay){
					...
					// Remove the existing study
					delete stx.layout.studies[parameters.replaceID];
					// Remove any other studies that relied on the old panel ID name, for instance a moving average on RSI(14) is no
					// longer valid if the panel is now RSI(15)
					if(panelName!=parameters.replaceID){
						for(var s in stx.layout.studies){
							var st=stx.layout.studies[s];
							if(st.panel==parameters.replaceID)
			...
			if(this.axisLabel && !this.highlighted){
				if(this.name=="horizontal") {
					...
					if(panel.chart.xAxis.formatter){
						...
						if(myDate.getHours()!=0 || myDate.getMinutes()!=0)
		...
		*/
		...
		STXChart.version=["Version 2014-12-02"];
		...
		function STXChart(config){
			...
			this.yTolerance=50;			// Number of pixels the mouse needs to move in vertical direction to "unlock" vertical panning
			...
			this.yaxisLabelStyle="roundRectArrow"; // Set to either "roundRectArrow", "semiRoundRect", "roundRect" or "rect"
			...
			this.extendLastTick=true;	// When set to true, line and mountain charts are extended slightly in order to reduce whitespace at the right edge of the chart
